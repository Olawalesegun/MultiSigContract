...
contract Multisig {
    ...
    bool hasCreaterChangedQuorumNumber;
    ...
    struct QuorumChanges {
      uint256 proposedQuorumNumber;
      bool isQuorumToChange;
      address[] whoAgrees;
    }
    ...
    function updateQuorum(uint8 _quorum) external {
      ...
      signerQuorumProposal[msg.sender].isQuorumToChange = true;
      signerQuorumProposal[msg.sender].proposedQuorumNumber = _quorum;

      emit QuorumUpdateRequest(msg.sender, _quorum);
      
    }

    function approveQuorum() external {
      require(msg.sender != address(0), "Address Zero Detected");
      require(msg.sender != creator, "You Already Approved");
      require(isValidSigner[msg.sender], "Not a Valid Signer");

      if(signerQuorumProposal[creator].isQuorumToChange) {
        signerQuorumProposal[creator].whoAgrees.push(msg.sender);
      }

      uint256 lengthOfSigners = signerQuorumProposal[creator].whoAgrees.length;
...
//       for(uint i = 0; i<isValidSigner.length)
//     }
}